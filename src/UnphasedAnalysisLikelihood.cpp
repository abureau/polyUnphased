/* UnphasedAnalysisLikelihood.cpp - Association analysis in trios and unrelateds
   ...
   UniversitÈ Laval
   DÈpartement de mÈdecine sociale et prÈventive
   1050 rue de la MÈdecine
   QuÈbec (QuÈbec), G1V 0A6, Canada
...
void UnphasedAnalysis::gradientFreeParameters(vector<double> &gradient,
    ...
    int iy = betasize;
    // freq
    // On en profite pour compter les haplotypes non-z√©ro et non-r√©f√©rence
    int nvhap = 0;
    for (int i = 0; i < nhap; i++) if (!zero[i] && i != refIndex) {
            g[ix--] = gradient[i+iy];
            nvhap++;
...
double UnphasedAnalysis::getloglikelihood(const string &title,
    ...
    valarray<double> bestFamilyCount[8], bestUnrelatedCount[2];
    for (int i = 0; i < 8; i++)
    	bestFamilyCount[i].resize(size);
    ...
    for (int restart = 0; restart <= options.restarts; restart++) {
        ...
        if (restart == 0 || loglikelihood > maxllhd) {
    		...
    		{
    			for (int i = 2; i < 8; i++)
            		bestFamilyCount[i] = familyCount[i];
			}    		 
    ...
    {
    		for (int i = 2; i < 8; i++)
            	familyCount[i] = bestFamilyCount[i];
	}    		 
