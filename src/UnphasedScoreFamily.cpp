...
void UnphasedAnalysis::scoreFamily(NuclearFamily &family, int nfamily,
    ...
    // sum of intercepts
    // Pas besoin de traiter le cas polytomique ici car les *Covariate0 sont dŽfinis seulement pour les phŽnotypes de type "quant".
    ...
    // Dans ce qui suit, on semble supposer que le trait est dichotomique sans l'avoir expressÃ©ment vÃ©rifiÃ©
                            ...
                            for (int sib = 0; sib < nsib; sib++)
                                if (!(options.chrX && parsex == MALE && sibSex[sib] == MALE)) {
                                    ...
                                    if (typeOfPhenotype == "polytomous") {
                                    	if (sibTrait[sib] < K) linear = betaparent[i + (sibTrait[sib]-1)*nhap];
                                    	}
        ...
        if (!sumZero) {
                    ...
                    for (int j = 0; j < 2; j++) {
                        if (typeOfPhenotype == "polytomous") {
                        	if (sibTrait[sib] < K) {
		                        ...
		                        linear[sib][i*2+j] += beta[whichHaps[1][i][j] + (sibTrait[sib]-1)*nhap];
		                        parentlinear[sib][i*2+j] += betaparent[whichHaps[1][i][j] + (sibTrait[sib]-1)*nhap];
		                        }
							}
            ...
            for (int sib = 0; sib < nsib; sib++)
                for (int v = 0; v < 4; v++) {
                	if (typeOfPhenotype == "polytomous") {
                		if (sibTrait[sib] < K) { 
                			virtualc[sib][v] += (1 - sumIntercept[sibTrait[sib]-1]) * linear[sib][v];
                			parentvirtualc[sib][v] += (1 - parentSumIntercept[sibTrait[sib]-1]) * parentlinear[sib][v];
                			}
                		}
                    else {
                    	virtualc[sib][v] += invVar * (sibTrait[sib] - sumIntercept[0]) * linear[sib][v];
                    	virtualc[sib][v] -= invVar * sumIntercept[0] * alphaterm[sib][v];
                    ...
                    if (invCov != 0)
                        for (int sib2 = 0; sib2 < nsib; sib2++) if (sib2 != sib) {
                                virtualc[sib][v] += invCov * (sibTrait[sib2] - sumIntercept[0]) * linear[sib][v];
                                virtualc[sib][v] -= invCov * sumIntercept[0] * alphaterm[sib][v];
    ...
    if (!(haveGlobal[0] && useSavedDenom[0]) &&
        ...
        if (!normal && !options.slow &&
            ...
            for (int i = 0; i < nhap; i++) if (!zero[i]) {
				    ...
				    betaparent0Gradient -= (invVar + (nsib - 1) * invCov) * betaparent[i] * derivTermft;
                                    if (options.hhrr) {
                                        betaparent0Gradient -= (invVar + (nsib - 1) * invCov) * alpha[i] * derivTermft;
                                    }
                                    for (int cov = 0; cov < betaCovariate.size(); cov++)
                                        for (int level = 0; level < betaCovariate[cov].size(); level++) {
				...
				betaparent0Gradient -= (invVar + (nsib - 1) * invCov) * betaparent[i] * derivTermmt;
                                if (options.hhrr) {
                                    betaparent0Gradient -= (invVar + (nsib - 1) * invCov) * alpha[i] * derivTermmt;
                                }
                                for (int cov = 0; cov < betaCovariate.size(); cov++)
                                    for (int level = 0; level < betaCovariate[cov].size(); level++) {
                                        ...
                                        if (positiveProbQueue[0]) {
                                            ...
                                            for (int sib = 0; sib < nsib; sib++) {
                                                ...
                                                if (!options.genotype && !MchrX) {
                                                    ...
                                                    betaparent0Gradient -= (invVar + (nsib - 1) * invCov) * betaparent[whichft] * x;
                                                    if (options.hhrr) {
                                                        betaparent0Gradient -= (invVar + (nsib - 1) * invCov) * alpha[whichft] * x;
                                                    }
                                                    for (int cov = 0; cov < betaCovariate.size(); cov++)
                                                        for (int level = 0; level < betaCovariate[cov].size(); level++) {
                                                ...
                                                betaparent0Gradient -= (invVar + (nsib - 1) * invCov) * betaparent[whichmt] * x;
                                                if (options.hhrr) {
                                                    betaparent0Gradient -= (invVar + (nsib - 1) * invCov) * alpha[whichmt] * x;
                                                }
                                                for (int cov = 0; cov < betaCovariate.size(); cov++)
                                                    for (int level = 0; level < betaCovariate[cov].size(); level++) {
    ...
    if (options.hhrr) {
        ...
        alpha0Gradient += betaparent0Gradient;
        for (int j = 0; j < betaCovariate.size(); j++)
            for (int k = 0; k < betaCovariate[j].size(); k++) {
        ...
        if (!haveGlobal[i] && useSavedDenom[i]) {
            ...
            if (haveBetaParent(options)) {
                ...
                if (haveBetaParent0(options)) {
                    globalGrad[i][ix++] += betaparent0Gradient;
                }
            }
        ...
        if (!sumZero) {
            ...
            for (int sib = 0; sib < nsib; sib++) {
                ...
                if (!options.genotype) {
                    ...
                    if (!options.chrX) {
                        if (!family.sibship) {
                        	if(typeOfPhenotype=="polytomous") familyCount[(int) sibTrait[sib] + K-1][whichHaps[0][1][1]] += sumX;
                        	else familyCount[0][whichHaps[0][1][1]] += sumX;
                        }
                    }
                }
                ...
                if (!family.sibship) {
                    if(typeOfPhenotype=="polytomous") familyCount[(int) sibTrait[sib] + K-1][whichHaps[1][1][1]] += sumX;
                	else familyCount[0][whichHaps[1][1][1]] += sumX;
                }
                ...
                for (int i = 0; i < 2; i++)
                    if (i == 1 || !options.genotype && !MchrX) {
                    	if (typeOfPhenotype == "polytomous") {
                    		if (sibTrait[sib] < K) betaGradient[whichHaps[i][0][0] + (sibTrait[sib]-1)*nhap] -= (1 - sumIntercept[sibTrait[sib]-1]) * sumX;
                    		}
                        else betaGradient[whichHaps[i][0][0]] -= invVar * (sibTrait[sib] - sumIntercept[0]) * sumX;
                        alphaGradient[whichHaps[i][0][0]] += invVar * sumIntercept[0] * sumX;
                        if (invCov != 0)
                            for (int sib2 = 0; sib2 < nsib; sib2++) if (sib2 != sib) {
                                    betaGradient[whichHaps[i][0][0]] -= invCov * (sibTrait[sib2] - sumIntercept[0]) * sumX;
                                    alphaGradient[whichHaps[i][0][0]] += invCov * sumIntercept[0] * sumX;
                                }
                        ...
                        if (typeOfPhenotype == "polytomous") {
	                        ...
	                        for (int j = 0; j < 2; j++)
	                            for (int k = 0; k < 2; k++) {
	                                linear[sib][j*2+k] += beta[whichHaps[i][j][k] + (sibTrait[sib]-1)*nhap];
	                                parentlinear[sib][j*2+k] += betaparent[whichHaps[i][j][k] + (sibTrait[sib]-1)*nhap];
								}
							}
						}
						else {                    		
                    ...
                    if (confounder[cov]) {
                        ...
                        for (int i = 0; i < 2; i++)
                            if (i == 1 || !options.genotype && !MchrX) {
                        		if (typeOfPhenotype == "polytomous") {
                                    if (sibTrait[sib] < K) { 
                                        betaCovariateGradient[cov][level][whichHaps[i][0][0] + (sibTrait[sib]-1)*nhap] -= (1 - sumIntercept[sibTrait[sib]-1]) * weight[i] * sumX;
                        				}
                        			}
                            	else {
                                betaCovariateGradient[cov][level][whichHaps[i][0][0]] -= invVar * (sibTrait[sib] - sumIntercept[0]) * weight[i] * sumX;
                                if (invCov != 0)
                                    for (int sib2 = 0; sib2 < nsib; sib2++) if (sib2 != sib) {
                                            betaCovariateGradient[cov][level][whichHaps[i][0][0]] -= invCov * (sibTrait[sib2] - sumIntercept[0]) * weight[i] * sumX;
                                        }
								}
			...
			/* Code de dÃ©buggage additionnel
            if (options.llhd) {
			...
			*/
            ...
            for (int sib = 0; sib < nsib; sib++)
                for (int v = 0; v < 4; v++) {
                	if (typeOfPhenotype == "polytomous") {
                	   if (sibTrait[sib] < K) virtualc[sib][v] += (1 - sumIntercept[sibTrait[sib]-1]) * linear[sib][v];
                	   }
                    else virtualc[sib][v] += invVar * (sibTrait[sib] - sumIntercept[0]) * linear[sib][v];
                    virtualc[sib][v] -= invVar * sumIntercept[0] * alphaterm[sib][v];
                    if (normal) {
                        virtualc[sib][v] -= invVar * 0.5 * linear[sib][v] * linear[sib][v];
                    }
                        ...
                        for (int j = 0; j < 2; j++)
                            for (int k = 0; k < 2; k++) if (!twoVirtualControls || j == k) {
                                    if (i == 1 || !options.genotype && !MchrX) {
                						...
                						// Pas de alpha0 avec phŽnotype polytomique, donc ne pas adapter ce calcul ne devrait pas avoir de consŽquence. 
                                        for (int cov = 0; cov < betaCovariate.size(); cov++)
                                            for (int level = 0; level < betaCovariate[cov].size(); level++) {
                                        ...
                                        if (typeOfPhenotype == "polytomous") {
                                        	if (sibTrait[sib] < K) betaparentGradient[whichHaps[i][j][k] + (sibTrait[sib]-1)*nhap] -= x[j*2+k];
                                        	}
                                        ...
                                        betaparent0Gradient += (invVar + (nsib - 1) * invCov) * betaparent[whichHaps[i][j][k]] * x[j*2+k];
                                        if (options.hhrr) {
    ...
    if (haveBetaParent(options)) {
        ...
        if (haveBetaParent0(options)) {
            gradient[ix++] += betaparent0Gradient;
